  ...
  CREATE TABLE case_action_audit (
   ...
   action VARCHAR(1) CHECK (action in ('R','U','C')) NOT NULL,
  ...
  comment on column case_action_audit.action is 'The action carried out-(R= View)';
  ...
  CREATE INDEX case_action_audit_action_idx ON case_action_audit (action);
